...
impl ImageConverter {
    ...
    pub(crate) fn create_slint_image_optimized(
        &self,
        rgba_data: &[u8],
        width: u32,
        height: u32,
        ...
        if rgba_data.len() == buffer_slice.len() {
            ...
            return Err(ImageConversionError::BufferSizeMismatch {
                source_size: rgba_data.len(),
                target_size: buffer_slice.len(),
            });
        }
...
pub enum ImageConversionError {
    ...
    #[error("Buffer size mismatch: source {source_size} bytes, target {target_size} bytes")]
    BufferSizeMismatch {
        source_size: usize,
        target_size: usize,
    },
